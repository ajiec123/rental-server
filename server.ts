    ...
    ws.on('message', (message: string) => {
      try {
        ...
        if (payload.type === 'REQUEST_SYNC') {
          ...
          const { stationId, customerName, customerPhone, durationMinutes, paymentMethod, amount, gamePlaying, cashierName } = payload.data;
          const now = Date.now();
          ...
          if (targetStation) {
            ...
            const newTx: Transaction = {
              ...
              durationMinutes,
              durationLabel: `${durationMinutes / 60 >= 1 ? `${Math.floor(durationMinutes / 60)} Jam` : ''} ${durationMinutes % 60 > 0 ? `${durationMinutes % 60} Mnt` : ''}`.trim(),
              paymentMethod,
              amount,
              formattedAmount: `Rp ${(amount / 1000).toLocaleString('id-ID')}k`,
              ...
              receiptNumber: 'NEX-' + new Date().toISOString().slice(0,10).replace(/-/g,'') + '-' + Math.floor(1000 + Math.random() * 9000)
            };
            ...
            stations = stations.map(st => {
              if (st.id === stationId) {
                return {
                  ...
                  totalSessionsToday: st.totalSessionsToday + 1,
                  totalRevenueToday: st.totalRevenueToday + amount
                };
              }
              return st;
            });
            ...
            broadcast({
              ...
              transactions,
              lastTransaction: newTx,
              actionMessage: `Timer dimulai WebSocket: ${targetStation.name} - ${customerName} (${durationMinutes} mnt)`
            });
